...
function BbsUpdate() {
	...
	const fileUpload = async (boardId) => {
		...
		await axios.post(`${process.env.REACT_APP_API_URL}/board/${boardId}/file/upload`, fd, {headers: headers})
			.then((resp) => {
	...
	const fileDelete = async (boardId, fileId) => {
		try {
			await axios.delete(`${process.env.REACT_APP_API_URL}/board/${boardId}/file/delete?fileId=${fileId}`, {headers: headers});
	...
	const updateBbs = async () => {
		...
		await axios.patch(`${process.env.REACT_APP_API_URL}/board/${bbs.boardId}/update`, req, {headers: headers})
		.then((resp) => {
